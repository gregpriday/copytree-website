<script>
	/** @type {{ variant?: 'diamonds' | 'triangles' | 'hexagons' | 'circles' | 'lines', size?: 'sm' | 'md' | 'lg', spacing?: 'tight' | 'normal' | 'loose', opacity?: number, animate?: boolean, class?: string }} */
	let {
		variant = 'diamonds',
		size = 'md',
		spacing = 'normal',
...
<div
	class="relative flex w-full items-center justify-center {className}"
...
>
	...
	<div class="relative flex items-center justify-center bg-background px-6">
		<div
			class="flex items-center gap-{spacing === 'tight'
				? '3'
				: spacing === 'loose'
					? '5'
					: '4'} {animate ? 'animate-pulse' : ''}"
		...
		>
			{#if variant === 'diamonds'}
				...
				{#each Array(3) as _, i}
					<div
						class="rotate-45 transform bg-gradient-to-br from-primary/60 to-emerald-500/60 transition-all duration-300 hover:from-primary hover:to-emerald-500"
						style="width: {currentSize.width}; height: {currentSize.height}; animation-delay: {animate
							? `${i * 200}ms`
							: '0ms'};"
					></div>
				{/each}
