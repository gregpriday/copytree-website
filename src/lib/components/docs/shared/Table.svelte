<script>
	/** @type {{ headers: string[], rows: any[][], class?: string, striped?: boolean, bordered?: boolean, compact?: boolean }} */
	let {
		headers,
		rows,
		class: className = '',
	...
	/**
	 * Helper function to determine if content is a snippet function
	 * @param {any} content
	 * @returns {boolean}
	 */
	function isSnippet(content) {
		return typeof content === 'function' && content.length === 0;
	}
</script>

<div
	class="w-full overflow-hidden rounded-lg {bordered
		? 'border border-border'
		: ''} bg-background/50 backdrop-blur-sm {className}"
>
