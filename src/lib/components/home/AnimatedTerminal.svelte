	...
	async function animateTerminal() {
		...
		for (let stepIndex = 0; stepIndex < commandSequence.length; stepIndex++) {
			...
			if (step.installing) {
				...
				visibleLines = (() => {
					const newLine = {
						text: step.installing,
						type: /** @type {'installing'} */ ('installing'),
						id: installingLineId
					};
					if (visibleLines.length >= 2) {
						return [visibleLines[1], newLine];
...
<div bind:this={terminalRef} class="my-8 w-full">
	<!-- Container adapts to theme using Tailwind's dark mode -->
	<div
		class="overflow-hidden rounded-lg border border-border bg-background/90 shadow-lg backdrop-blur-sm dark:shadow-none"
	>
		<!-- Terminal header - adapts to theme -->
		<div class="flex items-center gap-2 border-b border-border bg-secondary px-4 py-2">
			<div class="flex gap-1.5">
				<div class="h-3 w-3 rounded-full bg-red-500/80"></div>
				<div class="h-3 w-3 rounded-full bg-yellow-500/80"></div>
				<div class="h-3 w-3 rounded-full bg-green-500/80"></div>
			</div>
			<span class="ml-2 text-xs font-medium text-muted-foreground"> Terminal </span>
		</div>
				...
				{#each visibleLines as line, index (line.id)}
					...
					>
						...
						{:else}
							...
							<span
								class="ml-1 inline-block h-4 w-2 animate-pulse bg-emerald-400 transition-colors duration-150 {!isDone &&
								currentTypingLineId === line.id
									? 'opacity-100'
									: 'opacity-0'}"
