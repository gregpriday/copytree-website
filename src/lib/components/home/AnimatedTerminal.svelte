<script lang="ts">
  ...
  interface VisibleLine {
    text: string;
    type: 'prompt' | 'command' | 'output' | 'installing';
    id: number;
  }
  ...
  let visibleLines = $state<VisibleLine[]>([]);
  let isDone = $state(false);
  let currentTypingLineId = $state<number | null>(null);
  ...
  async function animateTerminal() {
    ...
    for (let stepIndex = 0; stepIndex < commandSequence.length; stepIndex++) {
      ...
      // Show the prompt
      visibleLines = (() => {
        const newLine = { text: prompt, type: 'command' as const, id: currentLineId };
      ...
      if (step.installing) {
        ...
        visibleLines = (() => {
          const newLine = { text: step.installing, type: 'installing' as const, id: installingLineId };
          if (visibleLines.length >= 2) {
            return [visibleLines[1], newLine];
          }
          return [...visibleLines, newLine];
        ...
        visibleLines = visibleLines.map(line => 
          line.id === installingLineId ? { ...line, text: step.output, type: 'output' as const } : line
        );
      } else {
        ...
        lineId++;
        visibleLines = (() => {
          const newLine = { text: step.output, type: 'output' as const, id: lineId };
          if (visibleLines.length >= 2) {
            return [visibleLines[1], newLine];
          }
          return [...visibleLines, newLine];
